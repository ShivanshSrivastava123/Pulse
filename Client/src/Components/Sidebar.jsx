...
const Sidebar = () => {
  ...
  const filteredData = input ? users.filter((user) => user.fullName.toLowerCase().includes(input.toLowerCase)) : users
        ...
        <div className='flex justify-between items-center'>
          ...
          <div className="relative py-2 group">
            ...
            <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block'>
              ...
              <p onClick={handlerFunction} className='cursor-pointer text-sm'>Logout</p>
            </div>
          </div>
        </div>
          ...
          filteredData.map((user , index) => (
            <div key={index} onClick={()=>{setSelectedUser(user); setUnseenMessages((prev) => ({...prev , [user._id] : 0}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
              ...
              <div>
                ...
                {/* random data */}
                {
                  onlineUsers.includes(user._id) ? <span className='text-green-700 text-xs'>Online</span> : <span className='text-neutral-400 text-xs'>Offline</span>
                }
              </div>
              {
